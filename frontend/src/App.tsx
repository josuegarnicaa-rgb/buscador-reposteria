        ...
        <main className="flex-1 py-8 sm:py-10">
          ...
          <section className="mt-8">
            ...
            {data?.resultados.length ? (
              <div className="mb-4">
                <h3 className="text-lg font-semibold text-stone-900">{textos.localResultsHeading}</h3>
              </div>
            ) : null}
            ...
            {data?.dbpedia?.length ? (
              <div className="mt-8">
                <h3 className="mb-4 text-lg font-semibold text-stone-900">{textos.dbpediaRemoteHeading}</h3>
                <div className="grid gap-5 lg:grid-cols-2">
                  {data.dbpedia.map((dbp) => (
                    ...
                    >
                      <div className="relative overflow-hidden bg-linear-to-br from-rose-100 via-orange-50 to-stone-100">
                        {dbp.imagen ? (
                          <img
                            src={dbp.imagen}
                            alt={dbp.nombre}
                        ...
                        <div className="absolute bottom-4 left-4 right-4">
                          <span className={`inline-flex rounded-full px-3 py-1 text-xs font-semibold uppercase tracking-[0.18em] ${claseOrigen(dbp.origen)} backdrop-blur`}>
                            {dbp.origen}
                          </span>
                      ...
                      <div className="space-y-5 p-5">
                        ...
                        <div className="flex items-center justify-between gap-3 border-t border-stone-100 pt-4">
                          <span className="text-xs text-stone-500">{textos.sourceLabel}</span>
                          <a
                            href={dbp.enlace}
                            target="_blank"
                            rel="noreferrer"
            ...
            {data?.dbpedia_local?.length ? (
              <div className="mt-8">
                <h3 className="mb-4 text-lg font-semibold text-stone-900">{textos.dbpediaLocalHeading}</h3>
                <div className="grid gap-5 lg:grid-cols-2">
                  {data.dbpedia_local.map((dbp) => (
                    <article
                      key={dbp.id}
                      className="overflow-hidden rounded-[1.75rem] border border-stone-200 bg-white shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg"
                    >
